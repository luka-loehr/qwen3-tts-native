...
for first_party in \
    'qwen3-tts-native 0.1.0' \
    'qwen3-tts-native-codec 0.1.0' \
    'qwen3-tts-runtime 0.1.0' \
    'qwen3-tts-server 0.1.0'
do
...
grep -Fq 'path+file:///source/native/qwen3-tts-server#0.1.0' "$SBOM" || \
...
jq -e --arg spec "$CYCLONEDX_SPEC_VERSION" '
    ...
    and (.metadata.component.version == "0.1.0")
