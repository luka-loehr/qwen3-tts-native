...
Only one subject may use CUDA during a run. The six stock runs execute first,
followed by one controlled container transition and the six Native runs. This
grouped order avoids repeated eviction and loading of the roughly 109~GB stock
working set, but it does not counterbalance persistent host-state or thermal
order effects; Section~\ref{sec:limitations} treats that as a limitation.
