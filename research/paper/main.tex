\documentclass[10pt]{article}

\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage[letterpaper,margin=0.82in]{geometry}
\usepackage{microtype}
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{array}
\usepackage{tabularx}
\usepackage{siunitx}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,shapes.geometric,fit,backgrounds,calc,decorations.pathreplacing}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usepackage[labelfont=bf,font=small,skip=6pt]{caption}
\usepackage{hyperref}

% Shared palette: colorblind-safe, legible in grayscale print.
\definecolor{papernavy}{HTML}{1F4E8C}   % native series, links, accents
\definecolor{papervermilion}{HTML}{C1521E} % stock comparator series
\definecolor{hostfill}{HTML}{EDF2F9}    % host / Rust plane
\definecolor{gpufill}{HTML}{EAF5EC}     % GPU / CUDA plane
\definecolor{gpuline}{HTML}{2E7D46}
\definecolor{neutralfill}{HTML}{F4F4F4}

\hypersetup{
  colorlinks=true,
  linkcolor=papernavy,
  citecolor=papernavy,
  urlcolor=papernavy,
  pdftitle={Qwen3-TTS Native: A Rust and CUDA Runtime for Streaming VoiceDesign Inference on NVIDIA DGX Spark},
  pdfauthor={Luka L\"ohr},
  pdfsubject={Native streaming text-to-speech inference},
  pdfkeywords={text-to-speech, Qwen3-TTS, Rust, CUDA, streaming inference}
}

\newcommand{\projectname}{Qwen3-TTS Native}
\newcommand{\modelname}{Qwen3-TTS-12Hz-1.7B-VoiceDesign}
\newcommand{\code}[1]{\texttt{#1}}
\newcommand{\EvidenceWarning}{%
  \begin{center}
    \fbox{\parbox{0.92\linewidth}{\centering\small
      Quantitative evidence is pending final validation. This source is a
      buildable research-paper scaffold, not a publishable result.}}
  \end{center}}

\input{data/evidence_placeholders}

\title{\textbf{Qwen3-TTS Native: A Rust and CUDA Runtime for\\
Streaming VoiceDesign Inference on NVIDIA DGX Spark}}
\author{Luka L\"ohr\\[3pt]
  \small\href{https://github.com/luka-loehr/qwen3-tts-native}{\texttt{github.com/luka-loehr/qwen3-tts-native}}}
\date{July 2026}

\begin{document}

\maketitle

\begin{abstract}
\input{sections/abstract}
\end{abstract}

\input{sections/introduction}
\input{sections/related_work}
\input{sections/system_design}
\input{sections/native_implementation}
\input{sections/streaming_api}
\input{sections/evaluation}
\input{sections/results}
\input{sections/limitations}
\input{sections/reproducibility}
\input{sections/broader_impacts}
\input{sections/conclusion}

\bibliographystyle{plain}
\bibliography{references}

\clearpage
\appendix
\input{sections/appendix}

\end{document}
